(() => {
  ...
  const componentSidebarLinks = componentItems.map(([id, label, icon]) =>
    `<a class="nav-link" href="component-${id}.html"><i class="bi ${icon}"></i><span>${label}</span></a>`
  ).join('');
  const componentHorizontalLinks = componentItems.map(([id, label, icon]) =>
    `<a class="dropdown-item" href="component-${id}.html"><i class="bi ${icon}"></i>${label}</a>`
  ).join('');
  ...
  if (location.pathname.endsWith('/components.html') || location.pathname.endsWith('components.html') || componentPageMatch) {
  ...
  if (chatApp) {
    ...
    const applyChatFilters = () => {
      ...
      if (visibleCount && (!activeContact || activeContact.hidden)) {
        contacts.find((contact) => !contact.hidden)?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
      }
    };
